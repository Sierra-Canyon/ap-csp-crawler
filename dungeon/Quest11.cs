// ======================================================================
//  Quest 11: Bars and Dots.   For 1, Wed 10/28
//  The sheet for this day has the rules and the rows it is graded on.
//  Check it:   cd ../dungeon-tests   then   dotnet run Quest11
//
//  QUEST NOT STARTED      <- delete this whole line when you begin the quest
// ======================================================================
string Stars(int n)
{
    return "";     // QUEST 11: replace this line
}

string ManaDots(int mana, int maxMana)
{
    return "";     // QUEST 11: replace this line
}

// ----------------------------------------------------------------------
//  Challenge 11: The Ruler.   Extra. Not graded.
//  CHALLENGE NOT STARTED  <- delete this whole line when you begin the challenge
// ----------------------------------------------------------------------
string Ruler(int width)
{
    return "";     // CHALLENGE 11: replace this line
}
